...
import webScraper
import player
from favoriteManager import *
import webbrowser

from webScraper import set_mp3s
from webScraper import set_list
...
def run():
    ...
    def searchTheArchive(year):#searches the archive looking for a shows in a certian year
        ...
        for i in range(num_items):
            ...
            label = tk.Button(canvas, text=item_name[i], bg="OliveDrab",command=lambda i=i :get_show_details(item_text[i]))
    ...
    def open_favs():
        ...
        x=Button(new_window,text="this is just a test!", bg="indian red")
        x.pack(side=TOP, fill=X)
    ()
